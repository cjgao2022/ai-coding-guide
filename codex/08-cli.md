...
![CC Switch 工具界面（占位图）](assets/08-cli/tui-three-zones.png)
...
**预期**：Codex 打印出当前会话的概况——用的哪个模型、审批策略是啥、有哪些可写目录、上下文还剩多少。**心里先有个底，知道自己现在是什么配置。也可以直接使用张哥的 token-tacker 项目显示如图**

![CC Switch 工具界面（占位图）](assets/08-cli/tui-output.png)

显示数据：项目名称/5 小时token剩余量/一周使用剩余量/上下文占比/当前模型
