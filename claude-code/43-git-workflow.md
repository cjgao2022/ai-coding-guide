...
翻成人话：**装了 `gh`，Claude 开 PR、读评论一条龙顺畅；不装，它退而用未认证的 GitHub API，动不动就被限流卡住。** 常见的情形是：一台新机器忘了装 `gh`，让它开 PR，它折腾半天报一堆 rate limit 错误——装上、`gh auth login` 登录一下，立刻丝滑。
...
> 一个典型场景：rebase 一个拖了两周的分支，十几个文件冲突，手动解到第五个就开始眼花、怕解错。交给 Claude 逐个分析后，它发现其中三个所谓的「冲突」其实是两边都做了同样的格式化、内容完全一致，直接告诉你「这几个随便选一边即可」，省了反复比对的功夫。
...
**你现在应该能：** 清清楚楚分出「本地、可逆」和「远端、收不回」两类 git 操作，把前者（看 diff、写 commit、开 PR、解冲突）放心交给 Claude 提效，把后者（push、force）的钥匙焊在自己手里（`deny` 规则）；知道装 `gh` CLI 能让它开 PR、读评论更顺，也记得 PR 评论可能藏提示注入；还能照着动手环节，在玩具仓库里把「diff → commit」这条链路独立跑通。**这套分工立起来，Claude 才是个让你 git 提效又不闯祸的好副手，而不是一个迟早替你按错「打款按钮」的隐患。**
