...
# 2. 会话期间临时切（运行不带参数的 /model 会打开选择器）
/model sonnet
```

```json
// 3. 写进 settings.json，永久作为新会话默认
{
  "model": "opus"
}
```

第四种是环境变量 `ANTHROPIC_MODEL=<别名或名称>`。一个实用的习惯：日常 `settings.json` 里固定 `sonnet`，碰到需要硬啃的架构问题，对话里临时 `/model opus` 顶上去，省额度。
